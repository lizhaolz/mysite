...
class LoginForm(forms.Form):
    username_or_email = forms.CharField(label='用户名或邮箱', required=True, widget=forms.TextInput(
        attrs={'class': 'form-control', 'placeholder': '请输入用户名或邮箱'}))
    password = forms.CharField(label='密码', widget=forms.PasswordInput(
        attrs={'class': 'form-control', 'placeholder': '请输入密码'}))  # widget是一个类，是我们前端页面要渲染的具体标签
...
class RegForm(forms.Form):
    ...
    password = forms.CharField(label='密码', min_length=6, required=True,
                               widget=forms.PasswordInput(attrs={'class': 'form-control', 'placeholder': '请输入密码'}))
    password_again = forms.CharField(label='再输入一次密码', required=True, widget=forms.PasswordInput(
    ...
    def clean_username(self):  # clean_+字段名称等于单独验证某个字段，是djandoform的自带方法,自动调用
...
class ChangeNicknameForm(forms.Form):
    nickname_new = forms.CharField(label='新的昵称', required=True,
    ...
    def clean_nickname_new(self):
        ...
        if nickname_new == 'lz':
            raise forms.ValidationError('昵称已经存在')
        return nickname_new
...
class ForgotPasswordForm(forms.Form):
    ...
    verification_code = forms.CharField(
        label='验证码',
        required=False,
        widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': '点击"发送验证码发送到邮箱"'})
    )
    new_password = forms.CharField(
        label='新密码',
